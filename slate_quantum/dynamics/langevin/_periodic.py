...
def solve_periodic_langevin[
    ...
    **kwargs: Unpack[RustSSEConfig],
    ...
    print(f"solve_periodic_langevin took: {(te - ts).total_seconds()} sec")  # noqa: T201
...
def solve_periodic_ballistic_langevin[
    ...
    **kwargs: Unpack[RustSSEConfig],
    ...
    ts = datetime.datetime.now(tz=datetime.UTC)
    data = sse_solver_py.solve_periodic_ballistic_langevin(  # type: ignore lib
        rescale_alpha(
            initial_state, in_parameter=parameters, out_parameter=normalized_params
        ),
        get_internal_parameters(
    ...
    te = datetime.datetime.now(tz=datetime.UTC)
    print(f"solve_periodic_ballistic_langevin took: {(te - ts).total_seconds()} sec")  # noqa: T201
    data = np.array(cast("list[complex]", data))  # pyright: ignore[reportUnnecessaryCast]
...
def solve_periodic_semiclassical_langevin[
    ...
    **kwargs: Unpack[RustSSEConfig],
    ...
    data = sse_solver_py.solve_periodic_semiclassical_langevin(  # type: ignore lib
        (
            rescale_alpha(
                initial_state[0],
                in_parameter=parameters,
    ...
    te = datetime.datetime.now(tz=datetime.UTC)
    print(  # noqa: T201
        f"solve_periodic_semiclassical_langevin took: {(te - ts).total_seconds()} sec"
    )
...
def solve_periodic_quantum_langevin[
    ...
    **kwargs: Unpack[RustSSEConfig],
    ...
    te = datetime.datetime.now(tz=datetime.UTC)
    print(f"solve_periodic_quantum_langevin took: {(te - ts).total_seconds()} sec")  # noqa: T201
