...
def solve_double_harmonic_langevin[
    ...
    **kwargs: Unpack[RustSSEConfig],
    ...
    print(f"solve_double_harmonic_langevin took: {(te - ts).total_seconds()} sec")  # noqa: T201
...
def solve_double_harmonic_ballistic_langevin[
    ...
    **kwargs: Unpack[RustSSEConfig],
    ...
    ts = datetime.datetime.now(tz=datetime.UTC)
    data = sse_solver_py.solve_double_harmonic_ballistic_langevin(  # type: ignore lib
        rescale_alpha(
            initial_state, in_parameter=parameters, out_parameter=normalized_params
        ),
        get_internal_parameters(
    ...
    te = datetime.datetime.now(tz=datetime.UTC)
    print(  # noqa: T201
        f"solve_double_harmonic_ballistic_langevin took: {(te - ts).total_seconds()} sec"
    )
    data = np.array(cast("list[complex]", data))  # pyright: ignore[reportUnnecessaryCast]
...
def solve_double_harmonic_semiclassical_langevin[
    ...
    **kwargs: Unpack[RustSSEConfig],
    ...
    data = sse_solver_py.solve_double_harmonic_semiclassical_langevin(  # type: ignore lib
        (
            rescale_alpha(
                initial_state[0],
                in_parameter=parameters,
    ...
    te = datetime.datetime.now(tz=datetime.UTC)
    print(  # noqa: T201
        f"solve_double_harmonic_semiclassical_langevin took: {(te - ts).total_seconds()} sec"
    )
