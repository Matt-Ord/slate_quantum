...
def _get_simulation_basis[M: EvenlySpacedLengthMetadata, E: AxisDirections](
    ...
    *,
    shape: tuple[int, ...],
) -> Basis[RepeatedVolumeMetadata[M, E]]:
    """Create a basis with n_repeat repeats of the periodic potential."""
    repeated_metadata = repeat_volume_metadata(metadata, shape=shape)
    return basis.transformed_from_metadata(repeated_metadata).upcast()
...
def simulate_caldeira_leggett_realizations[
    ...
    *,
    ...
    To avoid numerical instabilities, this function pre-calculates
    a suitable set of natural units for the simulation based on the Hamiltonian
    and initial state provided in the `condition`.
    ...
    simulation_basis = _get_simulation_basis(
        condition.potential.basis.metadata().children[0], shape=shape
    )
    hamiltonian = operator.build.kinetic_hamiltonian(
        condition.potential, condition.mass
    )
    # TODO: Build sin/cos operators
    ...
    result = qutip.ssesolve(  # type: ignore lib
        H=qutip.Qobj(
            hamiltonian.with_basis(operator_basis(simulation_basis)).raw_data.reshape(
                (simulation_basis.size, simulation_basis.size)
            )
            / hbar,
        ),
