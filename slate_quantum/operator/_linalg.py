...
def into_diagonal[M: BasisMetadata](
    ...
    return Operator(diagonal.basis.upcast(), diagonal.raw_data)
...
def into_diagonal_hermitian[M: BasisMetadata, DT: np.dtype[np.complexfloating]](
    ...
    return Operator(new_basis, diagonal.raw_data)  # type: ignore[return-value]
...
def get_eigenstates_hermitian[M: BasisMetadata, DT: np.dtype[np.complexfloating]](
    ...
    states = diagonal.basis.inner.inner.children[0].inner.eigenvectors()
    ...
    return StateList(out_basis, as_tuple.raw_data.astype(np.complex128))
...
def matmul[M0: BasisMetadata](
    ...
    return Operator(data.basis, data.raw_data)
