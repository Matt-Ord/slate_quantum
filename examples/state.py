...
if __name__ == "__main__":
    ...
    _, _, line = slate.plot.array_against_basis(position_state, ax=ax)
    line.set_label("Position Eigenstate")

    momentum_state = state.build.momentum(metadata, (20,))
    _, _, line = slate.plot.array_against_basis(momentum_state, ax=ax)
    line.set_label("Momentum Eigenstate")

    x_0 = 200 * 2 * np.pi / 500
    sigma_0 = 0.1 * np.pi
    coherent_state = state.build.coherent(metadata, (x_0,), (0,), (sigma_0,))
    _, _, line = slate.plot.array_against_basis(coherent_state, ax=ax)
    ...
    _, _, line = slate.plot.array_against_basis(function_state, ax=ax)
